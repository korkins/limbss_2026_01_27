...
6. The folder `tests` contains already extracted data for 4 cases (slides 2 and 3 in the presentation in `doc`). The 4 'txt' files are outputs (like in the menetioned `Iss_imod0_itst0_isol0_icom0_ialb0_iwav2_istk0.txt`). The Excel files contain benchmark data (column DZ) and results generated by `limbss` for 0, 1, and 2 auxiliary points (relative deviation is calculated and plotted there too). Specifically, for the default setup in the main py-file, one gets Iss = 3.989245e-02 at hkm_tp =  30.5 km and Zawada gives in `tst_imod0_itst0_isol0_icom0_ialb0_iwav2_istk0.xls`: column DZ, line 32, value = 0.039848 or 0.11 % error (a "funny thing": 0 auxiliary points is *more* accurate – 0.06% in this particular case !)  

7. `Iss[half:] = 2.250422e-02` at `hkm_tp =  30.5` (km) – this is left from the developer's tests: this number corresponds to integration along the nearest half of the LOS. In many cases, it is inaccurate. Just ignore it.

8. Yet another thing tested was how the source function (includes attenuation of the solar beam) and transmittances, respectively `Tsol = exp(-tau_sol)` and `Tlos = exp(-tau_los)`, look like as a function of 's' – location on the LOS ('s = 0' is the tangent point, 's > 0' – towards the sensor). Currently, `limbss(…)` returns these values (lines 105-106) for testing, but the final code may or may not retain these. Slide 15 of ppt in `doc` shows the figures for the default setup in `main_limbss.py`. Note the images are not saved as files, but displayed in the Python's IDE Spyder. Optionally, in line 156, replace plt.show() with plt.savefig(fname_Iss+“.jpg", dpi=300, bbox_inches="tight") to save the image into a file with the same name as the 'txt' (`fname_Iss` is defined in the code).
